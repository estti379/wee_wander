...
<x-layout :pageTitle=$pageTitle>
    @foreach ($event->adventures as $adventure)
        <div class="card">
            <h5 class="card-header" class="card text-center"><a href="/trail/{{ $adventure->trail->id }}">Event detail</a>
            </h5>
            <div class="card-body">
                <h2 class="card-title">{{ $event->title }}</h2> {{-- Event Title --}}
                <p>Trail title : {{ $adventure->trail->name }} Trail ID : </strong>{{ $adventure->trail->id }}</p>
                <p>Trail Organizer: <a href="/profile/{{ $event->username }}">{{ $event->organizer->firstname }}
