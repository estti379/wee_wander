...
<x-layout :pageTitle=$pageTitle>
    ...
    <form action="/events/{{ $event->id }}" method="POST">
        ...
        @for ($i = 0; $i < count($event->adventures); $i++)
            ...
            <br>
            <span>Due date : </span>
            <input type="date" name="due_date_E{{ $index }}"
                value="{{ old('due_date_E' . $index, $dueDateSplit[0]) }}">
            <label for="end_time">End Hour:</label>
            <input type="time" id="end_time_E{{ $index }}" name="end_time_E{{ $index }}"
        ...
        <div id="new-trail-inputs">
            @for ($i = 0; $i <= $newTrails; $i++)
                ...
                <div id="__{{ $i }}">
                    ...
                    <input type="date" name="starting_date__{{ $i }}"
                        value="{{ old('starting_date__' . $i) }}">
                    <label for="start_time__{{ $i }}">Starting Hour:</label>
                    <input type="time" name="start_time__{{ $i }}"
